...
var (
	...
	startTimeSchema = &jsonschema.Schema{
		Type:        "string",
		Description: "Start time as RFC3339 or Unix timestamp (optional)",
	}
	endTimeSchema = &jsonschema.Schema{
		Type:        "string",
...
var rangeQueryProperties = map[string]*jsonschema.Schema{
	"query": {
		Type:        "string",
		Description: "PromQL query string using metric names verified via list_metrics",
	},
...
func initExecuteRangeQuery() api.ServerTool {
	return api.ServerTool{
		Tool: api.Tool{
			...
			InputSchema: &jsonschema.Schema{
				Type:       "object",
				Properties: rangeQueryProperties,
...
func initShowTimeseries() api.ServerTool {
	props := make(map[string]*jsonschema.Schema, len(rangeQueryProperties)+2)
	maps.Copy(props, rangeQueryProperties)
